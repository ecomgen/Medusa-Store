...
export class Migration20260810103014 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`create table if not exists "bundle" ("id" text not null, "title" text not null, "description" text null, "is_active" boolean not null default true, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "bundle_pkey" primary key ("id"));`);
    this.addSql(`CREATE INDEX IF NOT EXISTS "IDX_bundle_deleted_at" ON "bundle" ("deleted_at") WHERE deleted_at IS NULL;`);

    this.addSql(`create table if not exists "bundle_item" ("id" text not null, "title" text not null, "quantity" integer not null, "bundle_id" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "bundle_item_pkey" primary key ("id"));`);
